...
Robot Framework est un framework open-source pour l’automatisation de tests basé sur des mots-clés (keywords). Il permet de réaliser : tests UI (web avec SeleniumLibrary), tests API, tests base de données, exécution automatisée en CI/CD et reporting avancé avec Allure ou ReportPortal.

⚙️ Prérequis

Installer Python depuis python.org

Vérifier la version et pip :
...
Extensions VS Code recommandées : RobotCode, Rainbow CSV, Robocorp Code, Python
...
Avec CSV : créer data/jdd.csv, installer DataDriver et configurer le test pour lire le CSV.
...
Exécution parallèle avec Pabot : créer trois fichiers .txt :
